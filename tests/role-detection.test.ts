...
const ROLE_CASES: RoleCase[] = [
  ...
  { name: 'Foo-Sources.tar.gz', hardRole: 'source', roles: ['source'] },
  { name: 'Foo-Windows-x64-PDBs.zip', hardRole: 'symbols', roles: ['symbols'] },
  { name: 'Foo-macOS-Apple-dSYMs.tar.xz', hardRole: 'symbols', roles: ['symbols'] },
  { name: 'foo-x86_64-dbsym.ddeb', hardRole: 'symbols', roles: ['symbols'] },
  { name: 'foo-linux-x86_64.appimage.zsync', hardRole: 'updater-metadata', roles: ['updater-metadata'] },
  { name: 'dist-manifest.json', hardRole: 'updater-metadata', roles: ['updater-metadata'] },
  { name: 'SHA256SUMS.txt', hardRole: 'checksum', roles: ['checksum'] },
  { name: 'SHA2-256SUMS', hardRole: 'checksum', roles: ['checksum'] },
  { name: 'README.md', hardRole: 'metadata', roles: ['metadata'] },
  { name: 'npm-audit.json', hardRole: 'metadata', roles: ['metadata'] },
  { name: 'config.yml', hardRole: 'metadata', roles: ['metadata'] },
  { name: 'foo-installer.sh', hardRole: null, roles: ['script'] },
  { name: 'foo-installer.ps1', hardRole: null, roles: ['script'] },
];
...
const KIND_CASES: KindCase[] = [
  ...
  { name: 'PowerToysUserSetup-0.100.2-x64.exe', kind: 'windows-installer', weight: 34 },
  { name: 'foo-websetup.exe', kind: 'windows-installer', weight: 34 },
  { name: 'foo-installer64.exe', kind: 'windows-installer', weight: 34 },
  { name: 'foo.msixbundle', kind: 'windows-installer', weight: 32 },
